	...
	public class CatmullRom
	{
		...
		public void DrawSpline(Color color)
		{
			if (ValidatePoints())
			{
				for (int i = 0; i < splinePoints.Length; i++)
				{
					...
					{
						UnityEngine.Debug.DrawLine(splinePoints[i].position, splinePoints[0].position, color);
					}
					else if (i < splinePoints.Length - 1)
					{
						UnityEngine.Debug.DrawLine(splinePoints[i].position, splinePoints[i + 1].position, color);
					}
				}
			}
		}
		...
		public void DrawNormals(float extrusion, Color color)
		{
			if (ValidatePoints())
			{
				for (int i = 0; i < splinePoints.Length; i++)
				{
					UnityEngine.Debug.DrawLine(splinePoints[i].position, splinePoints[i].position + splinePoints[i].normal * extrusion, color);
				}
			}
		}
		...
		public void DrawTangents(float extrusion, Color color)
		{
			if (ValidatePoints())
			{
				for (int i = 0; i < splinePoints.Length; i++)
				{
					UnityEngine.Debug.DrawLine(splinePoints[i].position, splinePoints[i].position + splinePoints[i].tangent * extrusion, color);
				}
			}
		}
		...
		public CatmullRomPoint GetPoint(float s)
		{
			...
			var currentPoint = Mathf.FloorToInt(s * (controlPoints.Length - 1));
			var endPoint = Mathf.CeilToInt(s * (controlPoints.Length - 1));
			if (s == 0)
			{
				endPoint = 1;
			}
			if (s == 1)
			{
				currentPoint = controlPoints.Length - 2;
			}

			var startDelta = (float)currentPoint / ((float)controlPoints.Length - 1);
			var endDelta = (float)endPoint / ((float)controlPoints.Length - 1);

			var pointDelta = MathUtils.MapAndClamp(startDelta, endDelta, 0, 1, s);
			...
			CatmullRomPoint point = Evaluate(p0, p1, m0, m1, pointDelta);
			return point;
